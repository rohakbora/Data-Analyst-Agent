...
def call_llm(input_text, model="gpt-4.1-mini"):
    ...
    try:
        ...
        logging.error(f"AIPipe LLM call failed: {e}")
        raise

SIZE_LIMIT = 100_000  # 100KB

def replace_image_paths_with_base64(obj, temp_dir):
    logging.info(f"replace_image_paths_with_base64 called with obj type: {type(obj)}")

    def normalize_to_temp_dir(path, temp_dir):
        if isinstance(path, str) and path.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp')):
            candidate = os.path.join(temp_dir, os.path.basename(path))
            if os.path.isfile(candidate):
                logging.warning(f"Normalizing path {path} → {candidate}")
                return candidate
        return path
    ...
    if isinstance(obj, dict):
        ...
        if obj.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp')):
            # Normalize before checking
            file_path = normalize_to_temp_dir(obj, temp_dir)

            # Normal check
            if not os.path.isfile(file_path):
                # Direct fallback: check in {temp_dir}/{filename}
                alt_path = os.path.join(temp_dir, os.path.basename(obj))
                if os.path.isfile(alt_path):
                    logging.warning(f"File not found at {file_path}, using fallback {alt_path}")
                    file_path = alt_path
                else:
                    logging.warning(f"Image file not found at both {file_path} and {alt_path}")
                    return obj  # Exit early, path doesn't exist anywhere
            ...
            try:
                ...
                with Image.open(file_path) as img:
                    ...
                    img = img.resize((max(1, int(width * scale_factor)),
...
PLOTS:
PLOTS (MANDATORY RULES):
- Save every plot as a PNG file to '{temp_dir}' only (no subfolders). Use a deterministic filename per plot, e.g. '{temp_dir}/network_graph.png', '{temp_dir}/degree_histogram.png'.
- Do NOT use BytesIO or base64 in code. Always write files to disk with plt.savefig(filepath, dpi=150, bbox_inches='tight', facecolor='white'), then plt.close().
- Always call plt.tight_layout() before save (ignore warnings if any).
- In your JSON, include the file paths (strings) exactly as saved, e.g. "{temp_dir}/degree_histogram.png".
- Never call tempfile.TemporaryDirectory(), and never write to any path outside '{temp_dir}'. If you call tempfile.TemporaryDirectory() inside your generated code, the images get deleted after execution is finished that is not acceptable.
- In JSON output, include the PNG file path as a string.
- Do NOT ever encode images as base64 or embed them in your code - the system will handle conversion.
            ...
            try:
                ...
                if code:
                    ...
                    messages = [
                        ...
                        {"role": "user", "content": f"The code output: {output[:2000]}...\nIf error, fix and provide new code. If successful, provide the FINAL JSON only using file paths for images saved at {temp_dir}, donot use base64, no code or extra text."}
                    ]
                else:
                    try:
                ...
                if iteration < max_iterations:
                    messages.append({"role": "user", "content": f"Previous attempt failed with error: {str(e)}. Retry with corrected approach. Save any images at {temp_dir}, donot bother returning base64 just the path at which image is saved. e.g {temp_dir}/image.png"})
                else:
                    break
